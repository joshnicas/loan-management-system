...
if (!isset($_SESSION['username'])) {
    header("Location:../employee-login.shtml"); // Redirect back if username is not set
    exit;
}
...
include("dbconfig.php"); // Include database connection
    ...
    $stmt = $conn->prepare("SELECT pwd, firstname, midname, lastname,contacts_1,contacts_2,unit,branch,indexnumber, profile_photo_path FROM loan_officer WHERE username = ?");
